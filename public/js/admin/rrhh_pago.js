const PagoModule = {
  ...
  cargarStats: async function () {
    try {
      ...
      if (document.getElementById("stat_pagados"))
        document.getElementById("stat_pagados").textContent = s.pagados || 0;
      if (document.getElementById("stat_pendientes"))
        document.getElementById("stat_pendientes").textContent =
          s.pendientes || 0;
      if (document.getElementById("stat_retrasados"))
        document.getElementById("stat_retrasados").textContent =
          s.retrasados || 0;
      if (document.getElementById("stat_monto_total"))
        document.getElementById("stat_monto_total").textContent =
          "S/ " + parseFloat(s.monto_total || 0).toFixed(2);
    } catch (e) {
      ...
      columns: [
        ...
        { data: "tipo" },
        {
          ...
          render: function (data, type, row) {
            ...
            if (data.length >= 7 && data.includes("-")) {
              ...
              if (m)
                return `<span class="text-capitalize">${m} ${partes[0]}</span>`;
            }
            return data;
          },
        },
        ...
        },
        {
          ...
          render: function (data) {
            ...
            if (data === "RECHAZADO")
              return '<span class="badge bg-label-secondary">Rechazado</span>';
        ...
        },
        {
          ...
          orderable: false,
          render: function (data, type, row) {
            if (row.estado === "PENDIENTE" || row.estado === "RETRASADO") {
              return `
                                <div class="d-flex justify-content-center gap-1">
                                    <button class="btn btn-sm btn-outline-success btn-pagar-trigger" data-id="${row.id_pago}" title="Marcar Pagado"><i class="bx bx-check"></i></button>
                                    <button class="btn btn-sm btn-outline-danger btn-rechazar-trigger" data-id="${row.id_pago}" title="Rechazar Pago"><i class="bx bx-x"></i></button>
                                </div>
                            `;
            } else if (row.estado === "RECHAZADO") {
              return `<span class="text-danger fs-4" title="Pago Rechazado"><i class="bx bx-check-double"></i></span>`;
            } else {
              // Pagado: 2 checks verdes como solicitaste
              return `<span class="text-success fs-4" title="Pago Confirmado"><i class="bx bx-check-double"></i></span>`;
            }
          },
        },
      ],
  ...
  initEventosUI: function () {
    ...
    if (offcanvasEl)
      this.filtroOffcanvas = new bootstrap.Offcanvas(offcanvasEl);
    ...
    $(document).on("click", ".btn-pagar-trigger", async function () {
    ...
    $(document).on("click", ".btn-rechazar-trigger", async function () {
      idPagoSeleccionado = $(this).data("id");
      const password = await window.confirmByPassword();
      if (password) {
    ...
    $("#btnConfirmarPago").on("click", function () {
      ...
      btn
        .prop("disabled", true)
        .html('<i class="bx bx-loader-alt bx-spin"></i> Procesando...');

      $.post(
        `${BASE_URL}/admin/pago/cambiarestado`,
        { id_pago: idPagoSeleccionado, estado: "PAGADO" },
        function (res) {
    ...
    $("#btnRechazarPago").on("click", function () {
      if (!idPagoSeleccionado) return;
      ...
      $.post(
        `${BASE_URL}/admin/pago/cambiarestado`,
        { id_pago: idPagoSeleccionado, estado: "RECHAZADO" },
        function (res) {
          btn.prop("disabled", false).text("Sí, rechazar");
          $("#modalRechazarPago").modal("hide");

          if (res.success) {
            self.tabla.ajax.reload(null, false);
            self.cargarStats();
            self.mostrarToast("Pago rechazado correctamente", "success");
            if (typeof window.updateGlobalNotifications === "function") {
              window.updateGlobalNotifications();
            }
          } else {
